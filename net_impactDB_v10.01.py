...
---------------------------------------------------
November 14, 2017 - vers 10
Created a simple print client function - prnt_clie_br
Testing how to implement/integrate this function
...
def add_actn_info():
    #print('enter data')
    clie = input('Enter last name like this -->  ')
    cur.execute('SELECT rowid,* FROM Dentist WHERE lnam LIKE ? ORDER BY lnam', ('%'+clie+'%',))

    result_clie = cur.fetchall()

    if result_clie == []:
        print('No client found.\n')
        return
    
    prnt_clie_br(result_clie)

    #for clie in result_clie:
      #  
     #   print(result_clie)
     #   prnt_clie_br(clie)

#Handles duplicate last names
    if len(result_clie) != 1:
            clie = (input("Enter first name followed by last name -->  ")).split()
            cur.execute('SELECT rowid,* FROM Dentist WHERE fnam LIKE ? AND lnam LIKE ? ORDER BY lnam', (clie[0], clie[1]))
            result_clie = cur.fetchall()
    ...
    status = input('Enter status (open or closed) --> ')

    log = input('Enter log info here --> ')

    action = input('Enter action here --> ')

    date2 = input('Enter the due date in this format: yyyy-mm-dd --> ')

    cur.execute("Select datetime('now','localtime')")
    #this is the exact time and date for this entry
    date1 = cur.fetchone()[0]


    cur.execute("INSERT INTO Actions VALUES(?, ?, ?, ?, ?, ?)", (date1, date2, clie[0], log, action, status))
...
#*# added this function
def prnt_clie_br(clie):
    """
    This function is a simple print function selecting fields from the client table and putting them
    in order.  You have to pass in a list of rows (tuples) from the client table (Dentist)
    """
    cntr = 3
    clie_lst = []
    for item in clie:
        clie_lst.append(item)
        if len(clie_lst) == cntr:
            #set up tabbing to align columns
            tab1 = "\t\t\t"
            tab2 = "\t\t\t"
            if (len(clie_lst[0][3]+", "+clie_lst[0][1])) >= 29:
                tab1 = "\t"
            elif (len(clie_lst[0][3]+", "+clie_lst[0][1])) >= 16:
                tab1 = "\t\t"
            if (len(clie_lst[1][3]+", "+clie_lst[1][1])) >= 16:
                tab2 = "\t\t"
            elif (len(clie_lst[1][3]+", "+clie_lst[1][1])) <= 7:
                tab2 = "\t\t\t\t"
            #tabbing set up
            print(clie_lst[0][3]+", "+clie_lst[0][1]+tab1+clie_lst[1][3]+", "+clie_lst[1][1]+tab2+clie_lst[2][3]+", "+clie_lst[2][1])  #+"\t\t"+clie_lst[3][1]+", "+clie_lst[3][3])
            clie_lst = []

    

    if len(clie_lst) == 2:
        for item in clie_lst:
            #set up tabbing to align columns
            tab1 = "\t\t\t"
            if (len(clie_lst[0][3]+", "+clie_lst[0][1])) >= 29:
                tab1 = "\t"
                ...
                tab1 = "\t\t"
            #tabbing set up
        print(clie_lst[0][3]+", "+clie_lst[0][1]+tab1+clie_lst[1][3]+", "+clie_lst[1][1])
    elif len(clie_lst) == 1:
        print(clie_lst[0][3]+", "+clie_lst[0][1])
...
#Handles duplicate last names
    ...
    status = input("Enter status you're looking for. --> ")

    cur.execute("SELECT * FROM Dent_Clin_Cross WHERE clie_id = ?", (clie[0][0], ))

    clin_id = cur.fetchall()
    #
    if clin_id == []:
        print("There is no clinic information.")
    else:
        cur.execute("SELECT * FROM Clinic WHERE rowid = ?", (clin_id[0][1], ))
        clin_dat = cur.fetchall()

    
    cur.execute("SELECT * FROM Actions WHERE clie_id = ? AND stat = ?", (clie[0][0], status))
    tab_dump = cur.fetchall()
    for act in tab_dump:
        print()
        print("Client:\n" + clie[0][1], clie[0][3])
        if clin_id != []: print("Clinic Info:\n" + clin_dat[0][0],' ',clin_dat[0][-6], ' ', clin_dat[0][-5])
        print('Log:', act[3])
        print('Action:', act[4])
        print('Due Date:', act[1][:11])
        print('Entry Date: ', act[0][:11], '  ', 'Status: ', act[-1])
...
def show_clie():
    ...
    '''
    #*# changed, added the order function
    cur.execute('SELECT rowid,* FROM Dentist ORDER BY lnam')
    return cur.fetchall()
...
#Connect to DB
con = sqlite3.connect('net_impactDB_v10TEST.db')
cur = con.cursor()
...
while cond_func == 'go':
    ...
    show_com = ['SHOW', '-'*18, 'show client', 'show clinic', 'show client/clinic', 'show action']
    find_com = ['FIND', '-'*18, 'find client', 'find clinic', '', '']
    add_com = ['ADD', '-'*18, 'add client', 'add clinic', 'add action', '']
    ...
    print('\nThis is the TESTING version.  Version 10TEST\n')
